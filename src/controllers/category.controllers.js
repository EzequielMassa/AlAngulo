...
export const deleteCategory = async (req,res) => {
    try{
        ...
        return res.status(200).json({ message : `Category with name ${categoryFound.name} was successfully erased` })
    } catch (error) {
...
export const updateCategory = async (req, res) => {
    try {
        ...
        return res.status(200).json({ message: `Category ${categoryOld} successfully updated`})
    } catch (error) {
        res.status(500).json({ message : error.message })
    }
}